...
@section('page_title', __('navia::generic.viewing').' '.__('navia::generic.settings'))
...
@section('content')
    <div class="container-fluid">
        ...
        @if(config('navia.show_dev_tips'))
        <div class="alert alert-info">
            <strong>{{ __('navia::generic.how_to_use') }}:</strong>
            <p>{{ __('navia::settings.usage_help') }} <code>setting('group.key')</code></p>
        </div>
        @endif
    </div>

    <div class="page-content settings container-fluid">
        <form action="{{ route('navia.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
                        ...
                        <div id="{{ \Illuminate\Support\Str::slug($group) }}" class="tab-pane fade in @if($group == $active) active @endif">
                            ...
                            <div class="panel-heading">
                                <h3 class="panel-title">
                                    {{ $setting->display_name }} @if(config('navia.show_dev_tips'))<code>setting('{{ $setting->key }}')</code>@endif
                                </h3>
            ...
            <button type="submit" class="btn btn-primary pull-right">{{ __('navia::settings.save') }}</button>
        </form>

        <div style="clear:both"></div>

        @can('add', Navia::model('Setting'))
        <div class="panel" style="margin-top:10px;">
            ...
            <div class="panel-body">
                <form action="{{ route('navia.settings.store') }}" method="POST">
                    ...
                    <input type="hidden" name="setting_tab" class="setting_tab" value="{{ $active }}" />
                    <div class="col-md-3">
                        <label for="display_name">{{ __('navia::generic.name') }}</label>
                        <input type="text" class="form-control" name="display_name" placeholder="{{ __('navia::settings.help_name') }}" required="required">
                    </div>
                    <div class="col-md-3">
                        <label for="key">{{ __('navia::generic.key') }}</label>
                        <input type="text" class="form-control" name="key" placeholder="{{ __('navia::settings.help_key') }}" required="required">
                    </div>
                    <div class="col-md-3">
                        <label for="type">{{ __('navia::generic.type') }}</label>
                        <select name="type" class="form-control" required="required">
                            <option value="">{{ __('navia::generic.choose_type') }}</option>
                            <option value="text">{{ __('navia::form.type_textbox') }}</option>
                            <option value="text_area">{{ __('navia::form.type_textarea') }}</option>
                            <option value="rich_text_box">{{ __('navia::form.type_richtextbox') }}</option>
                            <option value="markdown_editor">{{ __('navia::form.type_markdowneditor') }}</option>
                            <option value="code_editor">{{ __('navia::form.type_codeeditor') }}</option>
                            <option value="checkbox">{{ __('navia::form.type_checkbox') }}</option>
                            <option value="radio_btn">{{ __('navia::form.type_radiobutton') }}</option>
                            <option value="select_dropdown">{{ __('navia::form.type_selectdropdown') }}</option>
                            <option value="file">{{ __('navia::form.type_file') }}</option>
                            <option value="image">{{ __('navia::form.type_image') }}</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label for="group">{{ __('navia::settings.group') }}</label>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="new-settings-options">
                            ...
                            <div id="valid_options" class="alert-success alert" style="display:none">{{ __('navia::json.valid') }}</div>
                            <div id="invalid_options" class="alert-danger alert" style="display:none">{{ __('navia::json.invalid') }}</div>
                        </div>
                    </div>
                    <div style="clear:both"></div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="#" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger pull-right delete-confirm" value="{{ __('navia::settings.delete_confirm') }}">
                    </form>
...
@section('javascript')
    <script>
        $('document').ready(function () {
            ...
            $('.panel-actions .voyager-trash').click(function () {
                ...
                $('#delete_form')[0].action = '{{ route('navia.settings.delete', [ 'id' => '__id' ]) }}'.replace('__id', $(this).data('id'));
    ...
    $(".group_select_new").select2({
        tags: true,
        width: 'resolve',
        placeholder: '{{ __("navia::generic.select_group") }}'
    });
