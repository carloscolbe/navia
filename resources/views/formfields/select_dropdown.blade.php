@if(isset($options->relationship))
    ...
    @if( !method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        <p class="label label-warning"><i class="voyager-warning"></i> {{ __('navia::form.field_select_dd_relationship', ['method' => \Illuminate\Support\Str::camel($row->field).'()', 'class' => $dataType->model_name]) }}</p>
    @endif
    ...
    @if( method_exists( $dataType->model_name, \Illuminate\Support\Str::camel($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            @if(isset($options->options))
                <optgroup label="{{ __('navia::generic.custom') }}">
            ...
            <optgroup label="{{ __('navia::database.relationship.relationship') }}">
