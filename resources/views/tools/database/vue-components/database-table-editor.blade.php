...
<div class="panel panel-bordered">
    <div class="panel-body">
        ...
        @else
            <div class="col-md-6">
        @endif
                <label for="name">{{ __('navia::database.table_name') }}</label><br>
                <input v-model.trim="table.name" type="text" class="form-control" placeholder="{{ __('navia::database.table_name') }}" required pattern="{{ $db->identifierRegex }}">
            </div>

        @if($db->action == 'create')
            <div class="col-md-3 col-sm-4 col-xs-6">
                <label for="create_model">{{ __('navia::database.create_model_table') }}</label><br>
                <input type="checkbox" name="create_model" data-toggle="toggle"
                       data-on="{{ __('navia::generic.yes_please') }}" data-off="{{ __('navia::generic.no_thanks') }}">
            </div>
            {{--
                Hide migration button until feature is available.
                 <div class="col-md-3 col-sm-4 col-xs-6">
                    <label for="create_migration">{{ __('navia::database.create_migration') }}</label><br>
        ...
        <div v-if="compositeIndexes.length" v-once class="alert alert-danger">
            <p>{{ __('navia::database.no_composites_warning') }}</p>
        </div>

        <div id="alertsContainer"></div>

        <template v-if="tableHasColumns">
            <p>{{ __('navia::database.table_columns') }}</p>

            <table class="table table-bordered" style="width:100%;">
                <thead>
                <tr>
                    <th>{{ __('navia::generic.name') }}</th>
                    <th>{{ __('navia::generic.type') }}</th>
                    <th>{{ __('navia::generic.length') }}</th>
                    <th>{{ __('navia::generic.not_null') }}</th>
                    <th>{{ __('navia::generic.unsigned') }}</th>
                    <th>{{ __('navia::generic.auto_increment') }}</th>
                    <th>{{ __('navia::generic.index') }}</th>
                    <th>{{ __('navia::generic.default') }}</th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
        ...
        <div v-else>
          <p>{{ __('navia::database.table_no_columns') }}</p>
        </div>
    ...
    <div class="panel-footer">
        <input type="submit" class="btn btn-primary pull-right"
               value="@if($db->action == 'update'){{ __('navia::database.update_table') }}@else{{ __('navia::database.create_new_table') }}@endif"
...
@include('navia::tools.database.vue-components.database-column')
@include('navia::tools.database.vue-components.database-table-helper-buttons')
        ...
        methods: {
            addColumn(column) {
                ...
                if (column.name && this.hasColumn(column.name)) {
                    return toastr.error("{{ __('navia::database.column') }} " + column.name + " {{ __('navia::database.already_exists') }}");
                }
            ...
            renameColumn(column) {
                ...
                if ((existingColumn = this.getColumn(newName)) && (existingColumn !== column)) {
                    return toastr.error("{{ __('navia::database.column') }} " + newName + " {{ __('navia::database.already_exists') }}");
                }
            ...
            addIndex(index) {
                if (index.type == 'PRIMARY') {
                    if (this.table.primaryKeyName) {
                        return toastr.error("{{ __('navia::database.table_has_index') }}");
                    }
