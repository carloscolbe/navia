...
@section('page_title', __('navia::generic.menu_builder'))
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">{{ __('navia::menu_builder.drag_drop_info') }}</p>
                    </div>
                ...
                <div class="modal-footer">
                    <form action="{{ route('navia.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                        ...
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="{{ __('navia::menu_builder.delete_item_confirm') }}">
                    </form>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="" id="m_form" method="POST"
                      ...
                      data-action-update="{{ route('navia.menus.item.update', ['menu' => $menu->id]) }}">
                    ...
                    <div class="modal-body">
                        <div>
                            @include('navia::multilingual.language-selector')
                            <label for="name">{{ __('navia::menu_builder.item_title') }}</label>
                            @include('navia::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                            <input type="text" class="form-control" id="m_title" name="title" placeholder="{{ __('navia::generic.title') }}"><br>
                        </div>
                        <label for="type">{{ __('navia::menu_builder.link_type') }}</label>
                        <select id="m_link_type" class="form-control" name="type">
                            <option value="url" selected="selected">{{ __('navia::menu_builder.static_url') }}</option>
                            <option value="route">{{ __('navia::menu_builder.dynamic_route') }}</option>
                        </select><br>
                        <div id="m_url_type">
                            <label for="url">{{ __('navia::menu_builder.url') }}</label>
                            <input type="text" class="form-control" id="m_url" name="url" placeholder="{{ __('navia::generic.url') }}"><br>
                        </div>
                        <div id="m_route_type">
                            <label for="route">{{ __('navia::menu_builder.item_route') }}</label>
                            <input type="text" class="form-control" id="m_route" name="route" placeholder="{{ __('navia::generic.route') }}"><br>
                            <label for="parameters">{{ __('navia::menu_builder.route_parameter') }}</label>
                        ...
                        <label for="icon_class">{{ __('navia::menu_builder.icon_class') }} <a
                                    href="{{ route('navia.compass.index') }}#fonts"
                                    target="_blank">{!! __('navia::menu_builder.icon_class2') !!}</label>
                        <input type="text" class="form-control" id="m_icon_class" name="icon_class"
                               placeholder="{{ __('navia::menu_builder.icon_class_ph') }}"><br>
                        <label for="color">{{ __('navia::menu_builder.color') }}</label>
                        <input type="color" class="form-control" id="m_color" name="color"
                               placeholder="{{ __('navia::menu_builder.color_ph') }}"><br>
                        <label for="target">{{ __('navia::menu_builder.open_in') }}</label>
                        <select id="m_target" class="form-control" name="target">
                            <option value="_self" selected="selected">{{ __('navia::menu_builder.open_same') }}</option>
                            <option value="_blank">{{ __('navia::menu_builder.open_new') }}</option>
                        </select>
                        <input type="hidden" name="menu_id" value="{{ $menu->id }}">
                        <input type="hidden" name="id" id="m_id" value="">
                    </div>
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm__" value="{{ __('navia::generic.update') }}">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">{{ __('navia::generic.cancel') }}</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.add_item').click(function() {
                ...
                $m_form.find("input[type=submit]").val('{{ __('navia::generic.add') }}');
            ...
            $('.item_actions').on('click', '.edit', function (e) {
                $m_form.find("input[type=submit]").val('{{ __('navia::generic.update') }}');
            ...
            $('.item_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_form')[0].action = '{{ route('navia.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}'.replace('__id', id);
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('navia.menus.order_item',['menu' => $menu->id]) }}', {
                    order: JSON.stringify($('.dd').nestable('serialize')),
                    _token: '{{ csrf_token() }}'
                }, function (data) {
                    toastr.success("{{ __('navia::menu_builder.updated_order') }}");
                });
            });
        });
    </script>
